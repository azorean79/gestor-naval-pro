// Script: exportar_jangadas_template_excel.ts
// Descrição: Preenche o template Excel célula a célula para cada jangada/certificado, duplicando a folha para cada registo.
...
const OUTPUT_PATH = './export_jangadas_template.xlsx';
...
async function main() {
  const jangadas = JSON.parse(fs.readFileSync(DATA_PATH, 'utf8')) as JangadaTemplate[];
  const workbook = new ExcelJS.Workbook();
  await workbook.xlsx.readFile(TEMPLATE_PATH);
  const templateSheet = workbook.worksheets[0];

  // Para cada jangada/certificado, duplica a folha e preenche os campos
  jangadas.forEach((j: JangadaTemplate, idx: number) => {
    const sheet = workbook.addWorksheet(`Jangada ${j.serial || idx + 1}`);
    ...
    for (const [campo, cell] of Object.entries(cellMap)) {
      sheet.getCell(cell).value = (j[campo] as string | number | null | undefined) || '';
    }
    // Preencher artigos (exemplo: a partir da linha 20)
    let artigoRow = 20;
    (j.artigos || []).forEach((a: ArtigoTemplate) => {
  ...
  console.log('Exportação Excel (template) concluída:', OUTPUT_PATH);
}
