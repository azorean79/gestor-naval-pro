...
async function fetchPreview() {
  const validadeCandidates = await prisma.certificadoValidade.findMany({
    where: {
      OR: [
        { item: { contains: 'rampa', mode: 'insensitive' } },
        { item: { contains: 'ramp', mode: 'insensitive' } },
        { item: { contains: 'ladder', mode: 'insensitive' } },
        { item: { contains: 'escada', mode: 'insensitive' } },
      ],
    },
    include: {
      certificado: {
  ...
  const artigoCandidates = await prisma.artigoJangada.findMany({
    where: {
      validade: { not: null },
      OR: [
        { name: { contains: 'rampa', mode: 'insensitive' } },
...
async function applyCleanup(preview) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (artigoIds.length > 0) {
      const result = await tx.artigoJangada.deleteMany({
        where: { id: { in: artigoIds } },
      });
      deletedArtigos = Number(result.count || 0);
    }
...
async function main() {
  ...
  console.log('Pré-visualização da limpeza de itens sem validade gerida (Rampa / Boarding Ramp):');
  ...
  console.log(`- ArtigoJangada com validade a remover: ${preview.artigoRows.length}`);
  console.log(`- Jangadas afetadas: ${preview.affectedJangadaIds.length}`);
  console.log('- InspecaoArtigo não é alterado por este script para evitar apagar registos operacionais potencialmente manuais.');

  if (preview.validadeSample.length > 0) {
    console.log('\nAmostra de CertificadoValidade afetado:');
    preview.validadeSample.forEach((row) => {
      console.log(
        `  #${row.id} | cert=${row.certificadoNumero || row.certificadoId} | jangada=${row.raftSerial || '-'} | item=${row.item} | validade=${row.validade} | ficheiro=${row.fileName || '-'}`
      );
    });
  }
  ...
  console.log(`- ArtigoJangada removidos: ${result.deletedArtigos}`);
