...
from .port_movements import (
    DEFAULT_PORT_CODE,
    DEFAULT_PORT_MOVEMENT_LIMIT,
    PORTOS_DOS_ACORES_MOVEMENTS_URL,
    build_recent_activity_series,
    build_port_movements_response,
)
...
PORTOS_DOS_ACORES_CACHE_SECONDS = 15 * 60
# -------------------------
# HOME & AUTH
# -------------------------
...
class PortMovementSnapshotView(APIView):
    def get(self, request):
        port_code = request.GET.get("port", DEFAULT_PORT_CODE).upper()
        snapshot_date = request.GET.get("date")
        live = request.GET.get("live", "false").lower() == "true"
        archive = request.GET.get("archive", "false").lower() == "true"

        try:
            limit = int(request.GET.get("limit", DEFAULT_PORT_MOVEMENT_LIMIT))
        except (TypeError, ValueError):
            limit = DEFAULT_PORT_MOVEMENT_LIMIT

        limit = max(1, min(limit, 20))
        cache_key = f"port_movements:{port_code}:date:{snapshot_date or 'latest'}:limit:{limit}:live:{live}"
        cached_payload = cache.get(cache_key)
        if cached_payload and not live:
            return Response(cached_payload)

        try:
            payload = build_port_movements_response(
                port_code=port_code,
                limit=limit,
            ...
            if payload is None:
                return Response(
                    {
                        "port": {"code": port_code, "name": port_code},
                        "source": PORTOS_DOS_ACORES_MOVEMENTS_URL,
                        "error": f"Não existe arquivo diário para a data {snapshot_date}.",
                    },
                    status=status.HTTP_404_NOT_FOUND,
                )
            if not live:
                cache.set(cache_key, payload, PORTOS_DOS_ACORES_CACHE_SECONDS)
            ...
            return Response(
                {
                    "port": {"code": port_code, "name": port_code},
                    "source": PORTOS_DOS_ACORES_MOVEMENTS_URL,
                    "error": f"Não foi possível obter os movimentos portuários: {exc}",
                },
                status=status.HTTP_502_BAD_GATEWAY,
            )


class PortMovementActivityView(APIView):
    def get(self, request):
        port_code = request.GET.get("port", DEFAULT_PORT_CODE).upper()
        live = request.GET.get("live", "false").lower() == "true"
        archive = request.GET.get("archive", "false").lower() == "true"
        ...
        try:
            payload = build_recent_activity_series(
                port_code=port_code,
                days=days,
                live=live,
                archive=archive,
            ...
            return Response(
                {
                    ...
                    "error": f"Não foi possível gerar a atividade portuária: {exc}",
                },
                status=status.HTTP_502_BAD_GATEWAY,
            )
