...
const searchPoint = `      {/* Main Calendar View */}`;

const insertion = `  async function handleDesmarcarTodos() {
...
if (content.includes(searchPoint)) {
    // First remove the broken `<div className="flex h-screen bg-slate-50 overflow-hidden font-sans">` and `return (` that might be left
    content = content.replace(/return\s*\(\s*<div className="flex h-screen bg-slate-50 overflow-hidden font-sans">/, '');
    
    // Also remove stray `</aside>`
    content = content.replace(/<\/aside>/, '');
    
    // Now insert
    content = content.replace(searchPoint, insertion + searchPoint);
