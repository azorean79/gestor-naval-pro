...
/**
 * Brands that are *exempt* from the 3‑year rule. All other brands require the next
 * inspection to be scheduled three years after the inspection date.
 */
const threeYearBrands = ['RFD', 'DSB', 'ZODIAC'];

/**
 * Returns true if a given brand **needs** the 3‑year rule (i.e., it is NOT in the
 * exempt list). The check is case‑insensitive.
 */
export const needsThreeYearRule = (brand: string | undefined): boolean => {
  if (!brand) return false;
  return !threeYearBrands.includes(brand.toUpperCase());
};
