...
function getChildElements(parent: any, name?: string) {
  const result: any[] = [];
  ...
  for (let i = 0; i < parent.childNodes.length; i += 1) {
    ...
    if (!name || getLocalName(child) === name) result.push(child);
  }

  return result;
}

function findDescendants(root: any, name: string, acc: any[] = []) {
  if (!root?.childNodes) return acc;

  for (let i = 0; i < root.childNodes.length; i += 1) {
    const child = root.childNodes[i];
    if (child?.nodeType !== 1) continue;
    if (getLocalName(child) === name) acc.push(child);
...
function createWordElement(doc: any, tagName: string) {
  return doc.createElementNS(WORD_NS, `w:${tagName}`);
}

function removeElementChildren(parent: any, localName: string) {
  const children = getChildElements(parent, localName);
  children.forEach((child) => parent.removeChild(child));
}

function setCellText(cell: any, value: string) {
  const doc = cell.ownerDocument as any;
...
function createPageBreakParagraph(doc: any) {
  const paragraph = createWordElement(doc, "p");
  const run = createWordElement(doc, "r");
...
function getTopLevelTables(nodes: any[]) {
  return nodes.filter((node) => node?.nodeType === 1 && getLocalName(node) === "tbl");
}

function fillHeaderTable(table: any, input: NavioColetesCertificateInput) {
...
function fillRowsTable(table: any, rowsForPage: ColeteCertificateRow[]) {
  const rows = getChildElements(table, "tr");
  ...
  dataRows.forEach((row, index) => {
    ...
    setCellText(cells[4], current?.dataFabrico || "");
    setCellText(cells[5], current?.status || "");
  });
}

function fillFooterTable(table: any, input: NavioColetesCertificateInput) {
  const rows = getChildElements(table, "tr");
...
function fillPageContent(pageNodes: any[], input: NavioColetesCertificateInput, rowsForPage: ColeteCertificateRow[]) {
...
export async function generateNavioColetesCertificateDocx(input: NavioColetesCertificateInput) {
  ...
  pages.forEach((pageRows, pageIndex) => {
    const clonedNodes = baseNodes.map((node) => node.cloneNode(true));
