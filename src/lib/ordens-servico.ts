...
export type OrdemServicoTipo = "inspecao" | "manutencao" | "reparacao" | "outro";
...
export function normalizeOrdemStatus(value: unknown): OrdemServicoStatus {
  ...
  if (v === "em_progresso" || v === "progresso" || v === "em progresso") return "em_progresso";
...
export async function ensureOrderForServiceStation(params: {
      ...
      return tx.ordemServico.update({
        ...
        data: {
          ...
          metadados: toOrdemServicoMetaJson({
            ...appendWorkflowTransition(meta, nextWorkflowStatus, {
              origin: "queue",
              message: `Workflow atualizado a partir da estação para ${nextWorkflowStatus}.`,
  ...
  const numeroOrdem = await generateNumeroOrdem(expectedDate || new Date());
