...
const getStorageRootDir = () => {
  ...
  if (typeof path === "undefined" || typeof path.join !== "function") return "";
  
  try {
    const cwdVal = process.cwd();
    const cwd = typeof cwdVal === "string" ? cwdVal : "";
    const tmpVal = typeof os !== "undefined" && typeof os.tmpdir === "function" ? os.tmpdir() : "";
    const tmp = typeof tmpVal === "string" ? tmpVal : "";
    const namespace = APP_CONFIG?.storageNamespace || "default";

    if (process.env.VERCEL) {
      return path.join(tmp, namespace, "auditorias_documentos");
    }
    return path.join(cwd, "auditorias_documentos");
  } catch (err) {
    console.warn("Error resolving STORAGE_ROOT_DIR, using fallback:", err);
    return "";
  }
};
