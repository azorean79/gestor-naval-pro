...
function getCustomPackTypeModel() {
  return (prisma as any)?.customPackType;
}
...
export function isReservedPackName(_value: unknown) {
  return false;
}
...
function serializeCustomPack(pack: any): CustomPackTypeListItem {
  const filteredItems = Array.isArray(pack.items)
    ? pack.items.filter((item: any) => !isRaftManagedPackArticleName(item?.stockDescription))
    : [];
