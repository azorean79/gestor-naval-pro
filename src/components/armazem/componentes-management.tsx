...
import { Plus, Search, Edit, Trash2, Package, AlertTriangle, CheckCircle, Info } from 'lucide-react'
...
import { mapeamentoBoletins } from '@/lib/boletins-mapeamento'
...
export default function ComponentesManagement() {
                  ...
                  <TableRow key={componente.id}>
                    ...
                    <TableCell>
                      <div className="flex flex-col gap-1">
                        {/* Boletins aplicáveis */}
                        {(() => {
                          // Procura boletins onde o código antigo ou novo bate com o nome ou refFabricante
                          const codigos = [componente.nome, componente.refFabricante].filter(Boolean);
                          const boletins = mapeamentoBoletins.filter(b => codigos.includes(b.antigo) || codigos.includes(b.novo));
                          if (boletins.length === 0) return null;
                          return boletins.map((b, idx) => (
                            <Badge key={b.historico + b.antigo + idx} className={b.descontinuado ? 'bg-yellow-200 text-yellow-900' : 'bg-blue-100 text-blue-900'}>
                              <Info className="inline h-3 w-3 mr-1 align-text-bottom" />
                              {b.historico}: {b.descricao || `${b.antigo} → ${b.novo}`}
                            </Badge>
                          ));
                        })()}
