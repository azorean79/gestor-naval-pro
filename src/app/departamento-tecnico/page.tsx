...
type FolderType = 'manuais' | 'documentacao' | 'legislacao' | 'boletins';
...
type JangadaBrandCount = {
  marca: string;
  total: number;
};

type JangadaBrandModelCount = {
  marca: string;
  modelo: string;
  total: number;
};
...
function isGenericManualBrand(value: string): boolean {
  const normalized = normalizeToken(value || '').trim();
  return normalized === 'generica' || normalized === 'generic';
}

function buildBrandModelCountKey(brand: string, model: string): string {
  return `${normalizeToken(brand)}::${normalizeToken(model)}`;
}

function inferManualCategory(fileName: string, relativePath?: string): ManualCategory {
...
const FIXED_BRAND_ORDER = ["RFD", "DSB", "EUROVINIL", "SURVITEC", "ZODIAC", "SEA-SAFE", "LALIZAS", "VIKING", "PLASTIMO", "ARIMAR", "SEAGO", "OCEAN SAFETY"];
const MANUAIS_MAX_UPLOAD_BYTES = 50 * 1024 * 1024;
const DEFAULT_MAX_UPLOAD_BYTES = 500 * 1024 * 1024;
const EXTERNAL_ONLY_FOLDERS: FolderType[] = ['manuais', 'boletins'];
const INTERNAL_MANAGED_FOLDERS: Exclude<FolderType, 'manuais' | 'boletins'>[] = ['documentacao', 'legislacao'];

function getMaxUploadBytesByFolder(folder: FolderType): number {
  return folder === 'manuais' ? MANUAIS_MAX_UPLOAD_BYTES : DEFAULT_MAX_UPLOAD_BYTES;
}
...
function getModelKeyTechnicalDisplay(model: {
  name: string;
...
function getModelCompleteness(model: {
  inflationSystem?: string[];
  valves?: string[];
  torques?: string[];
  serviceItems?: Array<{ name: string; reference?: string }>;
...
function getSpecN2DisplayValue(spec: { cylinder?: { co2?: number; n2?: number } }) {
  if (typeof spec.cylinder?.n2 === 'number') return spec.cylinder.n2;
  if (typeof spec.cylinder?.co2 === 'number') return 0;
  return null;
}

function normalizeConfig(config?: string) {
...
function getItemSubsystem(item: TechnicalItem): string {
...
function getItemCriticality(item: TechnicalItem) {
...
function groupItemsBySubsystem(items: TechnicalItem[]) {
...
function getModelConsistencyIssues(model: {
  name: string;
  packTypes?: string[];
        ...
        <div className="rounded-lg border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900 text-center">
          ...
          <div className="mt-3 flex flex-wrap items-center justify-center gap-3 text-xs">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex rounded-lg border border-blue-300 bg-white px-3 py-2 font-medium text-blue-800 hover:bg-blue-100"
            >
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex px-4 py-2 text-sm font-medium rounded-lg transition-colors border border-blue-200 bg-white text-blue-700 hover:bg-blue-50"
                >
