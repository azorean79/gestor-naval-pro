...
type ClienteProfileField = "nome" | "numeroCliente" | "modoPagamento" | "morada" | "moradaNumero" | "codigoPostal" | "localidade" | "ilha" | "nif" | "email" | "telefone" | "telmovel";
type ClienteListColumnKey = "cliente" | "numeroCliente" | "ilha" | "email" | "telefone" | "navios";

const CLIENTE_LIST_COLUMNS_KEY = "clientes-list-columns-v1";
const CLIENTE_LIST_COLUMNS: Array<{ key: ClienteListColumnKey; label: string }> = [
...
function buildDefaultClienteListColumns(): Record<ClienteListColumnKey, boolean> {
...
const getMissingProfileFields = (cliente: Cliente): { key: ClienteProfileField; label: string }[] => {
    ...
    <div className="min-h-screen bg-slate-50 py-8">
      <div className="mx-auto flex max-w-[1500px] flex-col gap-6 px-4 sm:px-6 lg:px-8">
        ...
        {Object.keys(filteredGroupedClientes).length === 0 ? (
          ...
          <div className="overflow-auto rounded-xl border border-slate-200 bg-slate-50/50 p-4">
            ...
            <div className="flex items-center gap-2 mb-2">
              <button
                ...
                disabled={selectedClientes.length === 0 || deletingBatch}
                onClick={handleDeleteBatch}
              >
                {deletingBatch ? "A eliminar..." : `Excluir selecionados (${selectedClientes.length})`}
              </button>
              <span className="text-xs text-gray-500">Selecionados: {selectedClientes.length}</span>
            </div>
            <table className="min-w-full text-xs sm:text-sm">
              <thead>
                <tr className="bg-blue-100">
                  <th className="p-2"><input type="checkbox" onChange={e => handleSelectAllClientes(e.target.checked)} checked={selectedClientes.length > 0 && selectedClientes.length === filteredFlatClientes.length} /></th>
                  {isColumnVisible("cliente") && <th className="p-2">Cliente</th>}
                  {isColumnVisible("numeroCliente") && <th className="p-2">Nº Cliente</th>}
                  {isColumnVisible("ilha") && <th className="p-2">Ilha</th>}
                  {isColumnVisible("email") && <th className="p-2">Email</th>}
                  {isColumnVisible("telefone") && <th className="p-2">Telefone</th>}
                  {isColumnVisible("navios") && <th className="p-2">Navio(s)</th>}
                  <th className="p-2">Ações</th>
                </tr>
              </thead>
              <tbody>
                {filteredFlatClientes.map((cliente) => (
                  <tr key={cliente.id} className="border-t align-top">
                    <td className="p-2"><input type="checkbox" checked={selectedClientes.includes(cliente.id)} onChange={e => handleSelectCliente(cliente.id, e.target.checked)} /></td>
                    {isColumnVisible("cliente") && <td className="p-2 font-semibold"><Link href={`/clientes/${cliente.id}`} className="text-blue-700 hover:underline">{cliente.nome}</Link></td>}
                    {isColumnVisible("numeroCliente") && <td className="p-2">{formatNumeroCliente(cliente)}</td>}
                    {isColumnVisible("ilha") && <td className="p-2">{cliente.ilha || "Sem ilha"}</td>}
                    {isColumnVisible("email") && <td className="p-2">{cliente.email || "—"}</td>}
                    {isColumnVisible("telefone") && <td className="p-2">{cliente.telefone || cliente.telmovel || "—"}</td>}
                    {isColumnVisible("navios") && <td className="p-2 min-w-[220px]">
                      {cliente.navios.length > 0 ? (
                        <div className="flex flex-wrap gap-1">
                          {cliente.navios.slice(0, 3).map((navio) => (
                            <span
                              key={navio.id}
                              className="inline-flex items-center px-2 py-0.5 rounded bg-blue-50 text-blue-800 border border-blue-100"
                              title={`${navio.nome} (${navio.matricula})`}
                            >
                              {navio.nome}
                            </span>
                          ))}
                    ...
                    <td className="p-2 flex gap-2">
                      <button className="bg-blue-500 px-2 py-1 rounded text-xs text-white" onClick={() => setExpandedClienteId(cliente.id)}>Ver ficha</button>
                      <Link href={`/clientes/${cliente.id}`} className="bg-indigo-600 px-2 py-1 rounded text-xs text-white">Abrir página</Link>
                      <button className="bg-yellow-400 px-2 py-1 rounded text-xs" onClick={() => startEditCliente(cliente)}>Editar ficha</button>
                      <button className="bg-red-500 px-2 py-1 rounded text-xs text-white" onClick={() => handleDeleteCliente(cliente.id)}>Excluir</button>
                    </td>
                  </tr>
                ))}
                {filteredFlatClientes.length === 0 && (
                  <tr>
