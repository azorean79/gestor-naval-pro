...
const EVIDENCIAS_DIR_PREFIX = "jangadas-evidencias";
...
async function getValidatedJangada(id: number) {
  const access = await getAccessContext();
  if (!access) {
    return { access: null, jangada: null, error: NextResponse.json({ error: "Sessão obrigatória." }, { status: 401 }) };
  }

  const jangada = await prisma.jangada.findUnique({
    where: { id },
    select: { id: true, serviceStationId: true },
  });

  if (!jangada) {
    return { access, jangada: null, error: NextResponse.json({ error: "Jangada não encontrada." }, { status: 404 }) };
  }

  if (!access.isAdmin && !access.allowedStationIds.includes(Number(jangada.serviceStationId || 0))) {
    return { access, jangada, error: NextResponse.json({ error: "Sem permissão para aceder a esta jangada." }, { status: 403 }) };
  }

  return { access, jangada, error: null };
}
...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const { error } = await getValidatedJangada(id);
  ...
  const evidencias = files
    ...
    .map((file) => ({
      ...
      url: file.url || `/api/jangadas/${id}/evidencias?name=${encodeURIComponent(file.name)}`,
    }));
...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const { access, error } = await getValidatedJangada(id);
  if (error) return error;
  ...
  for (const file of files) {
    if (!isAllowedExtension(file.name)) {
      return NextResponse.json({ error: `Formato inválido: ${file.name}. Usa JPG, PNG ou WEBP.` }, { status: 400 });
    }
  ...
  for (const file of files) {
    ...
    uploaded.push({
      ...
      url: saved.url || `/api/jangadas/${id}/evidencias?name=${encodeURIComponent(saved.name)}`,
    });
  }
