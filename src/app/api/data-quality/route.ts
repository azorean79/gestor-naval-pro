...
export async function GET() {
  try {
    ...
    const [
      ...
      prisma.epirb.count({ where: { shipId: null } }),
      prisma.epirb.count({ where: { OR: [{ hexId: null }, { hexId: "" }] } }),
      prisma.epirb.count({ where: { OR: [{ dataValidadeBateria: null }, { dataValidadeBateria: "" }] } }),
    ]);
    ...
    const issues: QualityIssue[] = [
      ...
      },
      {
        ...
        href: "/epirbs",
        samples: epirbsSemNavio.map((item) => ({
          id: item.id,
          label: item.serial,
          meta: labelParts(item.marca, item.modelo),
          href: `/epirbs/${item.id}`,
      ...
      },
      {
        ...
        href: "/epirbs",
        samples: epirbsSemBateria.map((item) => ({
          id: item.id,
          label: item.serial,
          meta: labelParts(item.marca, item.modelo),
          href: `/epirbs/${item.id}`,
    ...
    return NextResponse.json({ error: "Erro ao gerar painel de qualidade de dados." }, { status: 500 });
  }
}