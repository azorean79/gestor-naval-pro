...
export async function GET(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    let epirbs: any[] = [];
    try {
      const epirbDelegate = (prisma as any).epirb;
    ...
    addDeadlineCandidate({
      ...
      entityType: 'navio',
      entityLabel: navio.nome,
      date: (navio as any).radarReflectorValidade,
      href: `/navios/${id}`,
      source: 'Ficha do navio',
    });

    parsePirotecnicos((navio as any).pirotecnicosBordoJson).forEach((item: any, index: number) => {
      addDeadlineCandidate({
    ...
    return NextResponse.json({ error: 'Erro ao buscar dados do navio', details: String(error) }, { status: 500 });
  }
}
...
export async function PUT(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (data.nome) {
      const conflictingNavio = await prisma.navio.findFirst({
        where: {
          id: { not: id },
          nome: {
            equals: String(data.nome),
            mode: 'insensitive',
          },
        },
      ...
      if (conflictingNavio) {
        return NextResponse.json({ error: `Já existe um navio com o nome ${conflictingNavio.nome}.` }, { status: 400 });
      }
    }
    ...
    return NextResponse.json({ error: 'Erro ao atualizar navio', details: String(error) }, { status: 500 });
  }
}
...
export async function DELETE(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({ error: 'Erro ao eliminar navio', details: String(error) }, { status: 500 });
  }
}
