...
type ChecklistRow = Prisma.OrdemServicoChecklistItemGetPayload<{
  select: {
    id: true;
    phase: true;
    category: true;
    ...
    updatedBy: true;
    updatedById: true;
    updatedByTecnico: { select: { nome: true } };
  };
}>;
...
async function fetchChecklistRows(ordemServicoId: number) {
  return prisma.ordemServicoChecklistItem.findMany({
...
function serializeChecklistRows(rows: ChecklistRow[]) {
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const updatedItem = await prisma.$transaction(async (tx) => {
      let target: { id: number; label: string | null; phase: string | null } | null = null;

      if (itemId) {
        target = await tx.ordemServicoChecklistItem.findFirst({
      ...
      if (target) {
        await tx.ordemServicoChecklistItem.update({
          where: { id: target.id },
          data: {
            ...(done !== undefined && { done }),
        ...
        target = await tx.ordemServicoChecklistItem.create({
          data: {
            ordemServicoId: id,
            phase,
