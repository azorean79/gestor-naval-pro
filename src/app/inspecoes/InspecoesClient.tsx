...
export default function InspecoesClient({ initialData }: { initialData: InspecaoData[] }) {
  ...
  const filteredData = useMemo(() => {
    ...
    return initialData.filter((insp) =>
  ...
  // Group by Month/Year (e.g. "junho 2026")
  ...
  const handleDownloadOrey = async (e: React.MouseEvent, insp: InspecaoData) => {
    ...
    try {
      // In the new architecture, the endpoint might need the raft ID and the inspection ID
      // If we don't have a direct endpoint, we can use the old endpoint or the new one if available.
      // Wait, let's just link to the wizard or open the certificate generation.
      // We will hit /api/certificados/orey?format=xlsx but we need the payload.
      // The payload is built inside the Wizard. For historical, we might need a dedicated endpoint or link.
      // For now, let's redirect to the Jangada Wizard, which is the standard way to interact.
      ...
      <div className="space-y-4">
        {Object.keys(groupedData).length === 0 ? (
          ...
          Object.entries(groupedData).map(([monthKey, inspecoes]) => {
            const isExpanded = expandedMonths[monthKey] !== false; // Default true or toggleable
                    ...
                    {inspecoes.map((insp) => {
                      ...
                      return (
                        <div key={insp.id} className="flex flex-col hover:bg-slate-50 transition-colors">
                              ...
                              <div>
                                ...
                                <div className="flex items-center gap-3 text-sm text-slate-500 mt-1">
                                  ...
                                  <span className="flex items-center gap-1">
                                    <Calendar size={14} /> {insp.dataInspecao}
                                  </span>
                                </div>
                              </div>
                            </div>
                          ...
                          {/* Expanded Row Content (Artigos e Testes) */}
                          {isRowExpanded && (
