            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {onlineUsers.map((user) => (
                ...
                >
                  ...
                  <p className="mt-3 text-xs text-emerald-800">Última atividade: {formatDateTime(user.presenceLastSeenAt)}</p>
                </button>
              ))}
            </div>
          )}
                    ...
                    filteredUsers.map((user) => (
                      ...
                      >
                        ...
                        <td className="px-3 py-2">
                          <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-semibold ${user.role === "ADMIN" ? "bg-blue-100 text-blue-800" : hasElevatedAccess({ role: user.role, permissions: user.permissions }) ? "bg-emerald-100 text-emerald-800" : "bg-gray-100 text-gray-700"}`}>
                        ...
                        <td className="px-3 py-2 text-gray-600">{formatDateTime(user.lastLoginAt)}</td>
              ...
              {!selectedUser ? (
                ...
                <div className="mt-4 space-y-3">
                  <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-sm text-slate-700">
                    ...
                    <p className="mt-2 text-xs text-slate-500">Última atividade detetada: {formatDateTime(selectedUser.presenceLastSeenAt)}</p>
                        ...
                        {selectedUser.activeSessions.map((activeSession) => (
                          <div key={activeSession.sessionId} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-slate-600">
                            <div className="flex flex-wrap items-center justify-between gap-2">
                              ...
                              <span>Ativa desde {formatDateTime(activeSession.createdAt)}</span>
                            </div>
                            <div className="mt-1">Última atividade: {formatDateTime(activeSession.lastSeenAt)}</div>
                  ...
                  <label className="block text-xs font-semibold text-gray-700">
                    ...
                    <select
                      ...
                      onChange={(e) => setSelectedUserForm((prev) => ({ ...prev, role: e.target.value as "ADMIN" | "USER" }))}
                      className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2"
                    >
                      <option value="USER">Utilizador</option>
                      <option value="ADMIN">Administrador</option>
