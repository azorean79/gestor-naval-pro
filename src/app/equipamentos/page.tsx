...
const COLETE_LIST_COLUMNS_KEY = "coletes-list-columns-v1";
const COLETE_LIST_COLUMNS: Array<{ key: ColeteListColumnKey; label: string }> = [
...
function buildDefaultColeteColumns(): Record<ColeteListColumnKey, boolean> {
...
function formatInspectionDate(value: string | null | undefined): string {
  if (!value) return "—";
...
function isInspectionOverdue(value: string | null | undefined): boolean {
  const normalized = normalizeDateInput(value);
  if (!normalized) return false;
  return new Date(`${normalized}T00:00:00`).getTime() < Date.now();
}

function isInspectionDueWithin30Days(value: string | null | undefined): boolean {
  const normalized = normalizeDateInput(value);
  if (!normalized) return false;
...
function getInspectionUrgencyRank(value: string | null | undefined): number {
...
function getUniqueNormalizedLabels(values: Array<string | null | undefined>): string[] {
...
export default function ColetesPage() {
  ...
  function getManualData(item: Colete) {
        ...
        <div className="app-hero-panel mb-6 flex flex-col gap-4 rounded-2xl p-6 text-white">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="flex flex-col items-end gap-2">
              <button className="rounded-lg bg-white px-4 py-2 text-base font-semibold text-blue-700 shadow-sm transition hover:bg-sky-50 w-full lg:w-auto" onClick={openNew}>
                + Cadastrar Novo Colete
              </button>
              <div className="flex flex-wrap gap-2 w-full justify-end">
                <Link href="/equipamentos/example-lalizas" className="rounded-lg bg-indigo-500 bg-opacity-40 border border-indigo-300 px-3 py-1.5 text-xs font-semibold text-white shadow-sm transition hover:bg-indigo-600">
                  Demo Lalizas
                </Link>
                <Link href="/equipamentos/example-eval" className="rounded-lg bg-indigo-500 bg-opacity-40 border border-indigo-300 px-3 py-1.5 text-xs font-semibold text-white shadow-sm transition hover:bg-indigo-600">
                  Demo EVAL
                </Link>
              </div>
            </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded-xl shadow-lg w-full max-w-md border border-gray-200">
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                ...
                <input name="dataInspecao" type="date" value={form.dataInspecao} onChange={handleChange} className="border rounded-lg px-3 py-2 w-full" />
                <input name="dataProxInspecao" type="date" value={form.dataProxInspecao} onChange={handleChange} className="border rounded-lg px-3 py-2 w-full" />
          ...
          <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded-xl shadow-lg w-full max-w-md border border-gray-200">
              ...
              <ul className="mb-4 text-sm space-y-1">
                ...
                <li><b>Data de inspeção:</b> {viewItem.dataInspecao || "-"}</li>
                <li><b>Próxima inspeção:</b> {viewItem.dataProxInspecao || "-"}</li>
                    ...
                    return (
                      <tr key={item.id} className="border-t align-top">
                        ...
                        <td className="p-2 flex gap-2">
                          <Link href={`/equipamentos/${item.id}`} className="bg-blue-500 px-2 py-1 rounded text-xs text-white">Ver ficha</Link>
