...
type PausedInspectionDraftMeta = {
  savedAt: number;
  inspectionWizardStep?: number;
};

type JangadaCatalogOption = {
  marca: string;
  modelo: string;
  source: "catalogo" | "departamentoTecnico" | "baseDados";
};

const INITIAL_FORM: Jangada = {
  id: 0,
  brand: "",
  model: "",
  launchType: "",
...
const FIRING_HEAD_KEYWORDS = ["head", "operating", "cabeca", "cabeça", "disparo", "firing", "gist", "dk99", "dk96", "leafield", "thanner", "nss", "vte", "hsr"];

function normalizeTechnicalSearchValue(value?: string | null) {
  return String(value || "")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase()
...
const JANGADA_LIST_COLUMNS_KEY = "jangadas-list-columns-v1";
const JANGADA_LIST_COLUMNS: Array<{ key: JangadaListColumnKey; label: string }> = [
  { key: "indice", label: "#" },
...
function buildDefaultJangadaColumns(): Record<JangadaListColumnKey, boolean> {
...
function getShortBulletinLabels(jangada: Jangada, limit?: number): string[] {
...
function isInspectionOverdue(value?: string): boolean {
  const parsed = Date.parse(String(value || ""));
  if (Number.isNaN(parsed)) return false;
  return parsed < Date.now();
}

function isInspectionDueWithin30Days(value?: string): boolean {
  const parsed = Date.parse(String(value || ""));
  if (Number.isNaN(parsed)) return false;
  const now = Date.now();
...
function isHydroTestOverdue(value?: string): boolean {
  const parsed = Date.parse(String(value || ""));
  if (Number.isNaN(parsed)) return false;
  return parsed < Date.now();
}

function isHydroTestDueWithin30Days(value?: string): boolean {
  const parsed = Date.parse(String(value || ""));
  if (Number.isNaN(parsed)) return false;
  const now = Date.now();
  const limit = now + 30 * 24 * 60 * 60 * 1000;
...
function isHruOverdue(value?: string): boolean {
  const parsed = Date.parse(String(value || ""));
  if (Number.isNaN(parsed)) return false;
  return parsed < Date.now();
}

function isHruDueWithin30Days(value?: string): boolean {
  const parsed = Date.parse(String(value || ""));
  if (Number.isNaN(parsed)) return false;
  const now = Date.now();
  const limit = now + 30 * 24 * 60 * 60 * 1000;
...
function isHruCritical(value?: string): boolean {
...
function getArticleStatusSummary(j: Jangada) {
  let expiredCount = 0;
  let upcomingCount = 0;
  const now = new Date();
...
function calculateComplianceSummary(j: Jangada) {
...
function normalizeMonthYearValue(value?: string | null) {
  if (!value) return "";
  const raw = String(value).trim();
  if (!raw) return "";
  if (/^\d{4}-\d{2}$/.test(raw)) {
...
function parseArticleValidityDate(value?: string | null) {
  const raw = String(value || "").trim();
  if (!raw) return null;
...
function getUpcomingReplacementArticles(jangada: Jangada) {
...
function aggregateUpcomingReplacementArticles(
...
function getInspectionUrgencyRank(value?: string): number {
  if (isInspectionOverdue(value)) return 0;
  if (isInspectionDueWithin30Days(value)) return 1;
  return 2;
}

function getPausedInspectionRank(meta?: PausedInspectionDraftMeta): number {
  return meta?.savedAt ? 0 : 1;
}

function loadPausedInspectionDraftsByRaft(): Record<number, PausedInspectionDraftMeta> {
...
function formatPausedInspectionLabel(savedAt: number): string {
...
function getInflationSystemLabel(jangada: Pick<Jangada, "brand" | "model">): string {
...
function hasAssociationValue(value?: string | null) {
  return Boolean(String(value || "").trim());
}

function getJangadaAssociationTone(jangada: Jangada): "green" | "yellow" | "red" | "neutral" {
...
function getJangadaAssociationRowClassName(jangada: Jangada): string {
...
function normalizeModelFilterKey(value: unknown): string {
  return String(value ?? "")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .replace(/\s+/g, " ")
...
function normalizeModelFilterLabel(value: unknown): string {
  return String(value ?? "")
    .replace(/\s+/g, " ")
    .trim()
    .toUpperCase();
...
function normalizeLaunchTypeValue(value?: string | null): string {
  const raw = String(value || "").trim();
  if (!raw) return "";
...
function getSuggestedLaunchType(configuration?: string[] | null) {
  return (configuration || [])
    .map((item) => normalizeLaunchTypeValue(item))
    .find(Boolean) || "";
}

function getSuggestedPackType(packTypes?: string[] | null) {
  return getRecognizedPackTypeOptions((packTypes || []).map((item) => String(item || "").trim())).find(Boolean) || "";
}

function getSuggestedCapacity(specifications?: Array<{ capacity: number }> | null) {
...
export default function JangadasPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!String(form.owner || "").trim()) {
      appToast.error("Indique o proprietário/cliente.");
      return;
    }
  ...
  const filteredJangadas = jangadas.filter(j => {
    ...
    return matchesSearch && matchesBrand && matchesModel && matchesPack && matchesExpiring && matchesHruCritical;
  });
        ...
        <div className="grid grid-cols-1 md:grid-cols-7 gap-2 items-end">
          <label className="block text-xs font-semibold text-gray-700 md:col-span-2">
            ...
            <input
              ...
              placeholder="Marca, modelo, série, proprietário ou navio"
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                ...
                <label className="block text-xs font-semibold text-gray-700">
                  ...
                  <input
                    ...
                    value={form.owner || ""}
                    onChange={handleChange}
                    list="jangada-owner-options"
                    placeholder="Nome do cliente ou proprietário"
                    className="mt-1 border rounded-lg px-3 py-2 w-full"
                    required
                  />
                  <datalist id="jangada-owner-options">
                ...
                <label className="block text-xs font-semibold text-gray-700">
                    Próxima inspeção
                    <input name="dataProxInspecao" type="date" value={form.dataProxInspecao || ""} onChange={handleChange} className="mt-1 border rounded-lg px-3 py-2 w-full" />
                  </label>
                    ...
                    <tbody>
                      {sortedFilteredJangadas.map((j, idx) => (
                        ...
                        >
                          ...
                          {isColumnVisible("dataFabrico") && <td className="p-2">{j.dataFabrico}</td>}
                    ...
                    sortedFilteredJangadas.map(j => (
                      ...
                      >
                        ...
                        <div className="text-xs text-gray-500 mb-1">
                          Data Fabrico: {j.dataFabrico || '—'} | Data Inspeção: {formatInspectionDate(j.dataInspecao)} | Próx. Inspeção:{' '}
