...
export default function CertificadosExternosPage() {
  ...
  const getErrorMessage = async (res: Response, fallback: string) => {
    ...
    if (data && typeof data === "object") {
      const body = data as { error?: string; message?: string; details?: string };
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- carregamento inicial de ficheiros.
    loadFiles();
    loadJangadas();
    // eslint-disable-next-line react-hooks/exhaustive-deps -- execução apenas no arranque; funções recriadas no corpo do componente.
  }, []);
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setUploadError(error instanceof Error ? error.message : "Erro ao enviar certificados.");
      setUploading(false);
    }
  };
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-4">
        ...
        {uploadError && (
          <div className="rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700 flex items-center justify-between">
            ...
            <button onClick={() => setUploadError(null)} className="ml-2 text-red-500 hover:text-red-700 font-bold">&times;</button>
          </div>
        )}
