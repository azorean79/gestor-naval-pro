...
type StockPriorityGroupKey = "validade" | "jangadas" | "restantes";
type StockScope = "all" | "jangadas-ocean";

type StockPrioritySection = {
...
const STOCK_NEW_ITEM_DRAFT_KEY = "stock-new-item-draft-v1";
const STOCK_LIST_COLUMNS_KEY = "stock-list-columns-v1";
const STOCK_CATEGORY_ACCORDIONS_KEY = "stock-category-accordions-v1";
const STOCK_SCOPE_KEY = "stock-scope-v1";

const STOCK_LIST_COLUMNS: Array<{ key: StockListColumnKey; label: string }> = [
  { key: "foto", label: "Foto" },
  { key: "nome", label: "Nome" },
...
function buildDefaultStockListColumns(): Record<StockListColumnKey, boolean> {
...
const INITIAL_STOCK_FORM: ItemStock = {
  id: 0,
  nome: "",
  quantidade: 0,
  referencia: "",
...
function parseMonthYearToDate(value?: string): Date | null {
...
function normalizeStockLabelText(value?: string | null) {
  return String(value || "")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase()
...
const CRITICAL_VALIDITY_CATEGORY_KEYWORDS = [
  "sinalizacao",
  "iluminacao",
  "primeiros socorros",
  "consumiveis",
...
export default function StockPage() {
  ...
  const [inlineEditDescricao, setInlineEditDescricao] = useState("");
    ...
    const loadFotos = async () => {
      ...
      for (let i = 0; i < ids.length; i += chunkSize) {
        ...
        try {
          ...
          // Ignora falhas pontuais de lote
        }
      }
  ...
  async function handleEdit(item: ItemStock) {
  ...
  function startInlineEdit(item: ItemStock) {
    setInlineEditId(item.id);
    setInlineEditNome(item.nome || "");
    setInlineEditDescricao(item.descricao || "");
  }

  function cancelInlineEdit() {
    setInlineEditId(null);
    setInlineEditNome("");
    setInlineEditDescricao("");
  }

  async function quickUpdateCategory(id: number, newCategory: string) {
    setLoading(true);
    setMessage(null);
    try {
      await requestJson(`/api/stock/${id}`, {
      ...
      setMessage({ type: "error", text: error?.message || "N�o foi poss�vel atualizar a categoria." });
    } finally {
      setLoading(false);
    }
  }
  ...
  async function saveInlineEdit(id: number) {
    ...
    try {
      await requestJson(`/api/stock/${id}`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ 
          nome: inlineEditNome.trim(),
          descricao: inlineEditDescricao.trim()
        }),
      });
      await fetchItens();
      ...
      setMessage({ type: "error", text: error?.message || "Não foi possível atualizar o artigo." });
    } finally {
      setLoading(false);
    }
  }
  ...
  const itensFiltrados = itens
    .filter((item) => {
      ...
      return okCategoria && okMarca && okModelo && okPack && okTexto && okStockBaixo && okValidade;
    })
    ...
    <div className="min-h-screen bg-slate-50 py-8">
      <div className="mx-auto flex max-w-[1500px] flex-col gap-6 px-4 sm:px-6 lg:px-8">
        <div className="app-hero-panel flex flex-col gap-4 rounded-2xl p-6 text-white">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button className="rounded-lg bg-white px-4 py-2 text-base font-semibold text-blue-700 shadow-sm transition hover:bg-sky-50" onClick={() => { fetchItens(); fetchNeedsAnalysis(); }}>
                Atualizar lista
              </button>
            </div>
          </div>
        ...
        <div className="grid gap-6 xl:grid-cols-[0.95fr_2.05fr]">
          <section id="stock-actions-panel" className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                <h2 className="text-xl font-semibold text-slate-900">{canEditStock ? "Operações rápidas" : "Consulta rápida"}</h2>
                <p className="text-base text-slate-600">
                  {canEditStock
                    ? "Atalhos para criar artigos, imprimir etiquetas e controlar seleção em lote."
                    : "Este perfil só pode consultar o stock. Criação, movimentos e edição estão desativados."}
                </p>
              </div>
              <div className="flex items-center gap-2">
                <span className={`rounded-full px-3 py-1 text-xs font-semibold ${canEditStock ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-700"}`}>
                  {canEditStock ? "Operacional" : "Só leitura"}
                </span>
                {canEditStock ? (
                  <button
                    type="button"
                    onClick={() => setIsActionsPanelExpanded((prev) => !prev)}
                    className="rounded-lg border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-700 transition hover:bg-slate-50"
                  >
                    {isActionsPanelExpanded ? "Recolher painel" : "Expandir painel"}
                  </button>
                ) : null}
              </div>
            </div>

            {canEditStock && isActionsPanelExpanded ? (
            <>
            <div className="mb-4 space-y-3 rounded-xl border border-slate-200 bg-slate-50 p-4">
              <button className="w-full rounded-lg bg-blue-700 px-4 py-2 text-sm font-semibold text-white" onClick={openCreateItem}>
                {editId ? "Continuar edição do artigo" : "Criar novo artigo"}
              </button>
              <div className="grid grid-cols-1 gap-2 text-xs text-slate-700">
                <div className="rounded-lg border border-slate-200 bg-white p-3">Artigos vencidos: <b>{stockStats.vencidos}</b></div>
                <div className="rounded-lg border border-slate-200 bg-white p-3">Selecionados para ações em lote: <b>{selectedIds.length}</b></div>
                <div className="rounded-lg border border-slate-200 bg-white p-3">Vista atual: <b>{viewMode}</b></div>
              </div>
            </div>
          ...
          <button
            ...
            onClick={() => printLabels(itensFiltrados)}
            disabled={itensFiltrados.length === 0}
          >
            Imprimir etiquetas (filtrados)
          </button>
          <div className="flex flex-wrap items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs">
            <span className="font-semibold text-gray-700">Etiquetas/Folha</span>
            <input
              type="number"
              min={1}
              max={120}
              value={etiquetasPorFolha}
              onChange={(e) => setEtiquetasPorFolha(Math.max(1, Number(e.target.value) || 1))}
              className="w-16 rounded border border-gray-300 px-2 py-1"
            />

            <span className="font-semibold text-gray-700">Colunas</span>
            <input
              type="number"
              min={1}
              max={8}
              ...
              onChange={(e) => {
                ...
                setColunasPorLinha(nextCols);
                setColunasUsadas((prev) => prev.filter((c) => c <= nextCols));
              }}
            ...
            <label className="inline-flex items-center gap-1 text-gray-700">
              <input
                type="checkbox"
                checked={folhaParcial}
                onChange={(e) => setFolhaParcial(e.target.checked)}
              />
              Folha já usada
            </label>
            ...
            {folhaParcial && (
              <div className="inline-flex items-center gap-1">
                <span className="text-gray-700">Colunas usadas (1ª linha):</span>
                {Array.from({ length: colunasPorLinha }, (_, idx) => idx + 1).map((col) => {
                  const checked = colunasUsadas.includes(col);
                  return (
                    <label key={col} className="inline-flex items-center gap-1 rounded border border-gray-300 px-1.5 py-0.5">
                      <input
                        type="checkbox"
                        checked={checked}
                        onChange={(e) => {
                          setColunasUsadas((prev) => {
                            if (e.target.checked) return Array.from(new Set([...prev, col])).sort((a, b) => a - b);
            ...
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-xs text-amber-900">
              ...
              <ul className="mt-2 space-y-1 list-disc pl-4">
                <li>Usa os filtros para isolar artigos com validade, packs e aplicabilidade a jangadas.</li>
                <li>As etiquetas respeitam a seleção filtrada atual.</li>
                <li>A análise de necessidades continua disponível no diretório à direita.</li>
              </ul>
            </div>
            </>
            ) : (
              <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 px-4 py-5 text-sm text-slate-600">
                {canEditStock
                  ? "O painel está recolhido para libertar espaço ao diretório. Abra-o quando precisar de criar um artigo ou usar ações rápidas."
                  : "Consulta disponível em modo só leitura. Pode filtrar, navegar e abrir as fichas dos artigos, mas não alterar dados nem movimentos."}
              </div>
            )}
          </section>
              ...
              <div className="mb-4 text-sm bg-gray-50 rounded-lg border border-gray-200 p-4">
                {isFichaEditing && fichaDraft ? (
                  <div className="space-y-3">
                    ...
                    <label className="block text-xs font-semibold text-gray-700">
                      Localização física
                      <input name="localizacao" value={fichaDraft.localizacao || ""} onChange={handleFichaFieldChange} className="mt-1 border rounded px-2 py-1 w-full" />
                    </label>
                    <label className="block text-xs font-semibold text-gray-700">
                      Observações
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                    ...
                    <div className="rounded-lg border border-gray-200 bg-white p-3"><span className="font-semibold text-gray-600">Localização</span><div className="text-sm text-gray-900 mt-1">{viewItem.localizacao || "-"}</div></div>
                ...
                <tr key={item.id} className="border-t align-top">
                  ...
                  {isColumnVisible("descricao") && <td className="p-2">
                    {inlineEditId === item.id ? (
                      <input
                        type="text"
                        value={inlineEditDescricao}
                        onChange={(e) => setInlineEditDescricao(e.target.value)}
                        className="border rounded px-2 py-1 w-full text-sm"
                  ...
                  <td className="p-2 flex gap-2">
                    {inlineEditId === item.id ? (
                      <>
                        <button className="bg-green-600 px-2 py-1 rounded text-xs text-white" onClick={() => saveInlineEdit(item.id)}>Guardar</button>
                        <button className="bg-gray-400 px-2 py-1 rounded text-xs text-white" onClick={cancelInlineEdit}>Cancelar</button>
                      </>
                    ) : canEditStock ? (
                      <>
                        ...
                        <button className="bg-purple-500 px-2 py-1 rounded text-xs text-white" onClick={() => startInlineEdit(item)}>Editar nome/desc</button>
                        <button className="bg-yellow-400 px-2 py-1 rounded text-xs" onClick={() => handleEdit(item)}>Editar ficha</button>
