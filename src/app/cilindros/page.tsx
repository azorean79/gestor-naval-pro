...
function normalizeCylinderSerialKey(value: unknown) {
...
export default function CilindrosPage() {
  ...
  const proximosTestesHidraulicos = useMemo(() => {
    ...
    return rows
      .map((row) => {
        const proxDate = parseDateValue(row.cylinderDataProxTeste);
              ...
              <tbody>
                {proximosTestesHidraulicos.map((row) => (
                  <tr key={`hydro-next-${row.id}`} className="border-t border-amber-100">
                    ...
                    <td className="px-2 py-1 font-semibold">{formatDatePt(row.cylinderDataProxTeste)}</td>
              ...
              filtered.map((row) => {
                return (
                  <tr key={row.id} className="border-t border-slate-100 align-top">
                    ...
                    <td className="px-3 py-2">{row.cylinderCo2 || "—"}</td>
                    <td className="px-3 py-2">{row.cylinderN2 || "—"}</td>
                    <td className="px-3 py-2">{row.cylinderDataTeste || "—"}</td>
                    <td className="px-3 py-2">{row.cylinderDataProxTeste || "—"}</td>
