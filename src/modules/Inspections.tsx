...
const SERVICE_TECHNICIANS = ["Julio Correa", "Alex Santos"] as const;
...
export default function Inspections() {
  ...
  const [quickInspection, setQuickInspection] = useState<QuickInspectionState>({ shipId: "", raftId: "", navioNome: "", jangadaSerial: "", date: "", status: "", responsavel: SERVICE_TECHNICIANS[0], certificadoNumero: "", certificateMode: "interno" });
  ...
  const emitAutomaticReports = async (
    ...
    anchor.download = response.headers.get("Content-Disposition")?.match(/filename="?([^";]+)"?/)?.[1]
      || `${[String(raftSerial || '').trim(), String(payload.brand || '').trim(), String(payload.raftModel || payload.model || '').trim(), (() => {
  ...
  const raftArticles = useMemo(() => {
    if (Array.isArray(selectedRaft?.artigos)) return selectedRaft.artigos;
    return [];
  }, [selectedRaft]);
  ...
  const getChecklistFieldStockConfig = (field: ChecklistField) => {
    const byName: Record<string, { refs?: string[]; articleTokens?: string[]; validityField?: string; quantity?: number; articleName?: string }> = {
      ...
      comprimidos_enjoo: { refs: ["30202051", "TAB-SICKNESS"], articleTokens: ["comprimidos", "enjoo", "seasickness tablets"], validityField: "validade_comprimidos", quantity: 1, articleName: field.label },
      foguetoes_paraquedas: { refs: ["20500023", "20577723"], articleTokens: ["paraquedas", "parachute rockets", "foguetoes", "foguetões"], validityField: "validade_paraquedas", quantity: 2, articleName: field.label },
      fachos_mao: { refs: ["20500035", "20577723"], articleTokens: ["facho", "fachos de mao", "fachos de mão", "handflares"], validityField: "validade_fachos_mao", quantity: 2, articleName: field.label },
  ...
  const handleQuickInspectionSubmit = async (e: React.FormEvent) => {
    ...
    const newInspection = {
      ...
      responsavel: quickInspection.responsavel || SERVICE_TECHNICIANS[0],
    ...
    setQuickInspection({ shipId: "", raftId: "", navioNome: "", jangadaSerial: "", date: "", status: "", responsavel: SERVICE_TECHNICIANS[0], certificadoNumero: "", certificateMode: "interno" });
  };
  ...
  useEffect(() => {
    ...
    const responsavel = (params.get("responsavel") || SERVICE_TECHNICIANS[0]).trim();
  ...
  const exportReportPDF = (email: string) => {
    const doc = new jsPDF();
  ...
  const exportReportExcel = (email: string) => {
    const filtered = inspections.filter((i: any) => i.responsavel === email);
      const rows = filtered.map((i: any) => {
      const ship = ships.find(s => s.id === i.shipId);
      const raft = rafts.find(r => r.id === i.raftId);
              ...
              {!isChecklistStart && (
                <>
                  ...
                  <select
                    ...
                    className="border rounded px-2 py-1 w-full text-gray-900 bg-white"
                  >
                    {SERVICE_TECHNICIANS.map((tecnico) => (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded shadow-lg w-full max-w-md">
            ...
            <form onSubmit={e => {
              ...
              <select name="responsavel" value={editModal.item.responsavel || SERVICE_TECHNICIANS[0]} onChange={e => setEditModal(m => ({...m, item: {...m.item!, responsavel: e.target.value}}))} className="border rounded px-2 py-1 w-full">
                {SERVICE_TECHNICIANS.map((tecnico) => (
                  <option key={tecnico} value={tecnico}>{tecnico}</option>
                ))}
              </select>
                  ...
                  {filteredStockItems.map(item => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {item.validade && (
                            <div className="text-xs text-blue-600 mt-1 font-medium">Val: {item.validade}</div>
                          )}
