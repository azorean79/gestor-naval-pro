...
export default function Step3_Componentes() {
  ...
  const addComponent = () => {
    setInspectionData({
      componentes: [
        ...componentes,
        { id: Date.now().toString(), type: COMPONENT_TYPES[0], reference: '', serialLote: '', validade: '' }
      ]
    });
  };
  ...
  const componentStock = (inspectionData.globalStock || []).filter((s: any) => 
    s.categoria?.toUpperCase().includes('VALVULA') || 
    s.categoria?.toUpperCase().includes('CILINDRO') ||
    s.categoria?.toUpperCase().includes('HRU') ||
    s.categoria?.toUpperCase().includes('MANGUEIRA') ||
    s.categoria?.toUpperCase().includes('COMPONENTE')
  );
