...
const { isKnownPackTypeName } = require('./src/lib/custom-pack-types');
...
async function main() {
  ...
  const rawInput = {
    ...existing,
    // Add the modifications that a user normally sends from the UI.
    // Let's assume they sent the editForm exactly as it is in the database initially
  };

  const jangadaData = { ...rawInput };
