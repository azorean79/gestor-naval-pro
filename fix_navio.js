const fs = require('fs');
const pf = 'src/components/navios/NavioAisLiveCard.tsx';
...
export default function NavioAisLiveCard({
  ...
  const canLookupRealtime = Boolean(navioId && String(mmsi || "").trim());
  ...
  const loadRealtimeData = useCallback(async (force = false) => {
    ...
    try {
      const response = await fetch(\/api/navios/\/ais-live\, {
        cache: "no-store",
      });
      ...
      setLiveData(payload);
      if (payload.vessel?.latitude && payload.vessel?.longitude) {
         setTrack(prev => {
  ...
  useEffect(() => {
    if (!canLookupRealtime || !autoRefresh || !navioId || !isPageVisible) {
      setStreamConnected(false);
      return;
    }

    const streamUrl = \/api/navios/\/ais-live/stream\;
    ...
    source.onmessage = (event) => {
      try {
        ...
        if (message.payload) {
          ...
          if (message.payload.vessel?.latitude && message.payload.vessel?.longitude) {
             setTrack(prev => {
                const lat = message.payload.vessel!.latitude!;
                const lng = message.payload.vessel!.longitude!;
                const last = prev[prev.length - 1];
  ...
  }, [autoRefresh, canLookupRealtime, isPageVisible, navioId]);
    ...
    <div className="bg-gradient-to-br from-blue-50 to-white rounded-2xl shadow-lg p-8 border border-blue-100 flex flex-col gap-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <span className={\inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold \\}>
          {hasRealtimeMapCoords ? "AISStream em tempo real" : hasLiveAisCoords ? "AIS com coordenadas" : "Fallback por porto/região"}
        </span>
      </div>
        ...
        <div className="rounded-xl border border-blue-100 bg-blue-50/50 p-6 space-y-4">
           {liveLoading && !liveData && (
             <div className="text-sm text-blue-700 font-medium animate-pulse flex items-center gap-2">
               ...
               A localizar o navio no AISStream...
             </div>
           )}
           {liveError && (
           ...
           {liveData?.noData && !liveError && (
             <div className="bg-amber-50 border border-amber-200 text-amber-800 p-4 rounded-lg text-sm">
               A API não devolveu posição recente para este MMSI. O navio pode estar fora de cobertura temporariamente.
             </div>
           )}
           {liveData?.vessel && (
             <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
               <div className="bg-white p-3 rounded-lg border border-slate-200">
                 ...
                 <div className="text-slate-900 font-bold">{liveData.vessel.course != null ? \\°\ : "N/D"}</div>
               </div>
               <div className="bg-white p-3 rounded-lg border border-slate-200">
                 <div className="text-slate-500 font-medium mb-1">Velocidade</div>
                 <div className="text-slate-900 font-bold">{liveData.vessel.speedKnots != null ? \\ kn\ : "N/D"}</div>
               </div>
               <div className="bg-white p-3 rounded-lg border border-slate-200">
           ...
           <div className="pt-2 flex items-center justify-between text-xs text-slate-500 space-x-2">
             <div className="flex items-center gap-2">
               ...
               <button
                 ...
                 className={\elative inline-flex h-5 w-9 shrink-0 cursor-pointer items-center justify-center rounded-full \\}
               >
                 <span className="sr-only">Ligar Atualização Stream</span>
                 <span aria-hidden="true" className={\pointer-events-none absolute left-0 inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out \\} />
               </button>
             </div>
             {streamConnected && autoRefresh ? (
                <span className="flex items-center gap-1.5 text-emerald-600 font-medium whitespace-nowrap">
                  ...
                  A escutar rede AIS... {track.length > 0 ? \(\ pings)\ : ""}
                </span>
        ...
        <>
          <div className="rounded-xl border border-slate-200 bg-white/80 p-4 text-sm text-slate-700 space-y-2">
            <p>
              {hasLiveAisCoords
                ...
                : \Quando ainda não existem coordenadas AIS utilizáveis, a ficha mostra uma referência geográfica estável em \ para manter contexto operacional no mapa.\}
            </p>
            <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-600">
              ...
              <span><strong>Fonte principal:</strong> AISStream</span>
            </div>
          </div>
              ...
              >
                <Popup>
                  <div className="text-sm">
                    ...
                    <div className="text-slate-600">{hasRealtimeMapCoords ? "Posição AISStream em tempo real" : hasLiveAisCoords ? "Posição AIS registada" : \Referência geográfica: \\}</div>
          ...
          <p className="text-xs text-slate-500">
            {hasRealtimeMapCoords
              ? "O mapa está a acompanhar automaticamente a posição AISStream e recentra quando chegam novas coordenadas."
              : hasLiveAisCoords
              ? "Se o navio mudar de posição no sistema, o mapa desta ficha acompanha as novas coordenadas após atualização dos dados."
              : \Enquanto o AISStream não devolve coordenadas utilizáveis, o mapa mantém uma referência geográfica estável em \.\}
          </p>
        </>
      )}
    </div>
